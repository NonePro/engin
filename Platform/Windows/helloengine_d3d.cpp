...
ID3D11Device *g_pDev = nullptr;         // the pointer to out Direct3D device interface
...
void InitPipeline()
{
    // load and compare the two shaders
    ID3DBlob *VS, *PS;
...
void InitGraphics()
{
    // crate a triangle using VERTEX struct
    Vertex OurVertices[] = {
    ...
    g_pDev->CreateBuffer(&bd, NULL, &g_pVBuffer); // crate the buffer
...
HRESULT CreateGraphicsResources(HWND hWnd)
{
    ...
    if (g_pSwapChain == nullptr)
    {
        ...
        if (hr == E_INVALIDARG)
        {
            hr = D3D11CreateDeviceAndSwapChain(NULL, D3D_DRIVER_TYPE_HARDWARE, NULL, 0, &FeatureLevelSupported, 1, D3D11_SDK_VERSION, &scd, &g_pSwapChain, &g_pDev, NULL, &g_pDevcon);
        }
...
void RenderFrame()
{
    ...
    // do 3D rendering on the back buffer here
    {
        ...
        // draw the vertex buffer on the back buffer
        g_pDevcon->Draw(3, 0);
    }
...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPTSTR lpCmdLine, int nCmdShow)
{
    ...
    hWind = CreateWindowEx(0,
                           _T("WindowClass1"),              // name of he window class
